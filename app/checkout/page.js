...
export default function CheckoutPage() {
  ...
  // PhonePe is only offered once real (production) credentials are wired in —
  // see NEXT_PUBLIC_PHONEPE_LIVE. Until then this defaults to Cash on
  // Delivery so nobody hits PhonePe's sandbox checkout by accident.
  const phonepeLive = process.env.NEXT_PUBLIC_PHONEPE_LIVE === "true";
  const [payMethod, setPayMethod] = useState(phonepeLive ? "phonepe" : "cod");
      ...
      {step === "payment" && (
        <>
          ...
          <div className="lt-pay-methods">
            <label className={"lt-pay-option" + (payMethod === "phonepe" ? " lt-active" : "") + (phonepeLive ? "" : " lt-pay-disabled")}>
              <input
                type="radio"
                name="pay"
                checked={payMethod === "phonepe"}
                disabled={!phonepeLive}
                onChange={() => setPayMethod("phonepe")}
              />
              <Smartphone size={16} /> PhonePe — UPI, Card, Net Banking
              {!phonepeLive && <span className="lt-pay-soon">Coming soon</span>}
            </label>
